...
export default function ModalVoucher({ open, openModal, closeModal, handleSubmitVoucher, isLoading, validationResult }: ModalVoucherProps) {
            ...
            <form onSubmit={handleSubmitVoucher} className={open ? "fixed inset-0 bg-black/70 flex items-center justify-center z-50" : "hidden"}>
                <div className="p-8 rounded-2xl bg-[#0D1A63]/80 border border-white/10" >
                    <div className="flex justify-end items-end">
                        <button type="button" onClick={closeModal} className="text-white/40 hover:text-white transition-colors text-sm font-medium">
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                    <div className="px-10 py-5 space-y-10">
                        ...
                        {validationResult?.message && (
                            <p className={validationResult.valid ? "text-green-300 text-sm" : "text-red-300 text-sm"}>
                                {validationResult.message}
                            </p>
                        )}
                        <button disabled={isLoading} type="submit" className="bg-white/11 w-full py-5 rounded-2xl font-syne font-bold text-base tracking-wide transition-all duration-500 flex items-center justify-center gap-3 disabled:opacity-30 disabled:cursor-not-allowed group/btn">{isLoading ? "Checking..." : "Apply"}</button>
                    </div>
                </div>
            </form>
        </>
